% % Discription: calculate the importance order of the 20 polarization
% images
% Date: 2019/01/25
% Author: Dongdong Ma
function calculate_acc_for_diff_lambda_beta_for_polar()

clc;
tic;
cls_name = {'duokong','guanghua','weizhu'};
base_path = 'E:\电纺丝数据\自己重新整理后\extracted_vec_for_all_cls\';
...
first_N = 30;                             % remain the first N elements
    ...
    for beta = 0:0.05:0.5
        ...
        for run = 1:run_num
            ...
            for ii = 1:length(cls_num)
                ...
                miu_mat = zeros(size(current_cls_data,2),MM_element_num);
                for j = 1:train_num(ii)
                    jj = train_index_set{1,ii}(j);
                    miu_mat = miu_mat + current_cls_data((jj-1)*MM_element_num+1:jj*MM_element_num,:)';
                end
                miu_mat = miu_mat/train_num(ii);
                miu_mat_set{1,ii} = miu_mat;
            end

            AA = zeros(MM_element_num,MM_element_num);
            BB = zeros(MM_element_num,MM_element_num);
            CC = zeros(MM_element_num,MM_element_num);
            cls_seq = [1,2,3];
            for ii = 1:length(cls_num)
                current_cls_name = strcat(base_path,cls_name{1,ii},'\',cls_name{1,ii},'_samples.mat');
                temp = load(current_cls_name);
                ...
                for j = 1:train_num(ii)
                    jj = train_index_set{1,ii}(j);
                    this_G_mat = current_cls_data((jj-1)*MM_element_num+1:jj*MM_element_num,:)';
            ...
            for ii = 1:length(cls_num)
                ...
                for j = 1:train_num(ii)
                    jj = train_index_set{1,ii}(j);
                    single_current_cls_data = current_cls_data((jj-1)*MM_element_num+1:jj*MM_element_num,:)';
            ...
            for ii = 1:length(cls_num)
                ...
                for jj = 1:length(this_hard_num_vec)
                    ...
                    this_G_mat = current_cls_data((new_jj-1)*MM_element_num+1:new_jj*MM_element_num,:)';
                    AA = AA + (this_G_mat - miu_mat_set{1,ii})'*(this_G_mat - miu_mat_set{1,ii});
                end
                % calculate BB
                for jj = 1:length(cls_num)
